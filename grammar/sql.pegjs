{
  ...
  function createUnaryExpr(op: string, e: string) {
    return {
      type: 'unary_expr',
      operator: op,
      expr: e
  ...
  function createBinaryExpr(op: string, left: string, right: string) {
    return {
      type: 'binary_expr',
      operator: op,
      left: left,
  ...
  function createList(head: any, tail: any) {
    const result = [head];
  ...
  function createBinaryExprChain(head: any, tail: any) {
    let result = head;
    for (let i = 0; i < tail.length; i++) {
...
rename_stmt
  = KW_RENAME  __
    KW_TABLE __
    t: table_to_list {
      t.forEach((tg: any[]) => tg.forEach((dt: any) => dt.table && tableList.add(`rename::${dt.db}::${dt.table}`)))
        return {
          tableList: Array.from(tableList),
          columnList: Array.from(columnList),
          ast: {
...
select_stmt_nake
  = cte:with_clause? __ KW_SELECT __
    ...
    l:limit_clause? {
      if(f) f.forEach((info: any) => info.table && tableList.add(`select::${info.db}::${info.table}`));
      return {
          with: cte,
          type: 'select',
          options: opts,
...
update_stmt
  = KW_UPDATE    __
    ...
    w:where_clause? {
      ...
      if(l) l.forEach((col: any) => columnList.add(`update::${t.table}::${col.column}`));
      return {
        tableList: Array.from(tableList),
        columnList: Array.from(columnList),
        ast: {
...
replace_insert_stmt
  = ri:replace_insert       __
    ...
    v:value_clause {
      ...
      if (c) c.forEach((c: any) => columnList.add(`insert::${t.table}::${c}`));
      return {
        tableList: Array.from(tableList),
        columnList: Array.from(columnList),
        ast: {
